          ...
          <tbody>
            @foreach($coupons as $coupon)
                <tr>
                    ...
                    <td>
                        @if($coupon->type=='fixed')
                            RWF{{number_format($coupon->value,2)}}
                        @else
                            {{$coupon->value}}%
                        @endif</td>
                    <td>
                    ...
                    <td>
                        <a href="{{route('coupon.edit',$coupon->id)}}" class="text-primary btn-sm float-left mr-1" style="height:30px; width:30px;border-radius:50%" data-toggle="tooltip" title="edit" data-placement="bottom"><i class="fas fa-edit"></i></a>
                        <form method="POST" action="{{route('coupon.destroy',[$coupon->id])}}">
                          @csrf
                          @method('delete')
                              <button class="text-danger dltBtn border" data-id={{$coupon->id}} style="height:30px; width:30px;border-radius:50%" data-toggle="tooltip" data-placement="bottom" title="Delete"><i class="fas fa-trash-alt"></i></button>
                        </form>
                    </td>
                    {{-- Delete Modal --}}
